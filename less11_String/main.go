...
func useStrings() {
	...
	fmt.Println("Подсчет вхождений подстроки в строку")
	s := "banana"
	count := strings.Count(s, "a")
	fmt.Println(count) // 3

	// Проверка наличия подстроки в строке
	fmt.Println("Проверка наличия подстроки в строке")
	s = "Hello, World!"
	coutains := strings.Contains(s, "World")
	fmt.Println(coutains) // true

	// Разделение строки на подстроки
	fmt.Println("Разделение строки на подстроки")
	s = "a,b,c"
	split := strings.Split(s, ",")
	fmt.Println(split) // ["a", "b", "c"]

	// Объединение строк с разделителем
	fmt.Println("Объединение строк c разделителем")
	parts := []string{"Go", "is", "great"}
	joined := strings.Join(parts, " ")
	fmt.Println(joined) // "Go is great"

	// Замена подстроки
	fmt.Println("Замена подстроки")
	s = "Hello, Gophers!"
